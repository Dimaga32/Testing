...
export default function RegisterContent() {
   ...
   const isShowRegister = useSelector(
      (state: TypeReducerState) => state.register.isShowRegister,
   );
   const handleClick = (e: MouseEvent): void => {
      if (
         RegistrationElement.current &&
         !RegistrationElement.current.contains(e.target as Node)
      ) {
            ...
            >
               ...
               <Button
                  ...
                  onClick={(e) => {
                     ...
                     if (
                        formData.current.name.length >= 5 &&
                        formData.current.name.length <= 20 &&
                        formData.current.email.includes("@") &&
                        formData.current.password.length >= 6
                     ) {
                        handlePostClick(
                           formData.current,
                           "http://localhost:5000/api/registers_persons",
                        );
                     }
