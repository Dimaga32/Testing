...
app.get("/api/user/usersDate", async (req: Request, res: Response):Promise<object|void> => {
    if (res.locals.newAccessToken) {
        res.setHeader('x-new-access-token', res.locals.newAccessToken);
    }

    try {
        const result:{rows:TypeData[]} = await pool.query("SELECT * FROM user_schema.users"); // Явное указание схемы
...
app.post("/api/registers_persons", async (req: Request, res: Response):Promise<void> => {
...
app.post("/api/login_persons", async (req: Request, res: Response):Promise<object|void> => {
    ...
    try {
        ...
        if (result.password===hash_password){
            ...
            return res.status(200).json({ accessToken, refreshToken,id ,name,email});
        }
        else {
...
app.get("/api/user/AccountInfo", async (req: Request, res: Response):Promise<object|void> => {
    if (res.locals.newAccessToken) {
        res.setHeader('x-new-access-token', res.locals.newAccessToken);
    }
    try {
        const result:{rows:TypeData} = await pool.query("SELECT * FROM user_schema.registers_persons WHERE id = $1",[req.user]);
